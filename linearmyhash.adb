...
begin
   ...
   Sum, HA : Integer;
   Percent : Integer;
   FMinPeeks : Integer := 9000;
   FMaxPeeks : Integer := 0;
   FAvgPeeks : Float := 0.0;
   FTotalPeeks : Integer := 0;
   LMinPeeks : Integer := 9000;
   LMaxPeeks : Integer := 0;
   LAvgPeeks : Float := 0.0;
   ...
   Sum :=  character'pos(data(2))
         * Character'Pos(Data(3))*3
         + character'pos(data(6))*6
         + Character'Pos(Data(1))
         + character'pos(data(7))*7;
   HA := abs(Sum) mod 128;
   Kountpeeks := 1;
   initialRandInteger;
   While(KountPeeks <= (2**n)) Loop
      --Put("Trip"); Put(KountPeeks,1); Put(" HA = "); Put(HA,0); New_Line;
      if HT(HA) = Data then
      ...
      for I in 1..30 loop
         ...
         Sum :=  character'pos(data(2))
               * Character'Pos(Data(3))*3
               + character'pos(data(6))*6
               + character'pos(data(1)) + character'pos(data(7))*7;
         HA := Abs(Sum) mod 128;
         Kountpeeks := 1;
         initialRandInteger;
         loop
